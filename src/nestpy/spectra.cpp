...
py::array_t<double>fill_spectra(
	...
	double m1,
	double m2,
	std::function<double(double, double, double, double)> spectra
){
	auto vec = std::vector<double>(number);
...
void init_spectra(py::module& m){
	...
	m_spectra.def("B8", 
		[](int number, double emin, double emax, double m1, double m2){return fill_spectra(number, emin, emax, m1, m2, &TestSpectra::B8_spectrum);},  
		py::arg("number"),
		py::arg("xMin") = 0.,
		py::arg("xMax") = 4.,
		py::arg("m1") = -2.6455e-10, 
